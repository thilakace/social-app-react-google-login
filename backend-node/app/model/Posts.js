...
const getAllList = (module, userId, res) => {
  ...
  let sqlQuery = "SELECT id,question,link,status,view_count,like_count,dislike_count,share_count,created_at,updated_at,created_by,author_name, DATE_FORMAT(updated_at, '%M,%d %Y %H:%i:%s') AS updated_at FROM posts where status!=? order by updated_at DESC";
